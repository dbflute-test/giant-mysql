 ...
 */
@AllowAnyoneAccess
public class WxThymeleafParadeAction extends GiantBaseAction {
    ...
    //                                                                             Execute
    //                                                                             =======
    // http://localhost:8151/giant/wx/thymeleaf/parade/
    @Execute
    public HtmlResponse index(OptionalThing<Integer> pageNumber, WxThymeleafSearchForm form) {
        validate(form, messages -> {}, () -> {
            return asHtml(path_WxThymeleaf_WxThymeleafParadeHtml);
        });
        PagingResultBean<Product> page = selectProductPage(pageNumber.orElse(1), form);
        List<WxThymeleafSearchRowBean> beans = page.stream().map(product -> {
            return mappingToBean(product);
        }).collect(Collectors.toList());
        return asHtml(path_WxThymeleaf_WxThymeleafParadeHtml).renderWith(data -> {
    ...
    //                                                                              Select
    //                                                                              ======
    private PagingResultBean<Product> selectProductPage(int pageNumber, WxThymeleafSearchForm form) {
    ...
    //                                                                             Mapping
    //                                                                             =======
    private WxThymeleafSearchRowBean mappingToBean(Product product) {
        WxThymeleafSearchRowBean bean = new WxThymeleafSearchRowBean();
