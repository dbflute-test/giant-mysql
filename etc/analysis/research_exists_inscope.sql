...
--                                                                  ======================
--
-- 基点テーブル(外側)の絞り込みが甘くて件数が多く、サブクエリ(内側)の絞り込みが強くて件数が少ない場合
--
...
-- (MySQL-5.6.14) => 想像を超える遅さ
-- o 素直すぎる DEPENDENT SUBQUERY !?
-- _/_/_/_/
+----------+
| count(*) |
+----------+
|      152 |
+----------+
1 row in set (6 min 48.38 sec)
+----+--------------------+-------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+---------+--------------------------+
| id | select_type        | table | type  | possible_keys                                                  | key                                    | key_len | ref                   | rows    | Extra                    |
+----+--------------------+-------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+---------+--------------------------+
|  1 | PRIMARY            | gi    | index | NULL                                                           | IX_GIANT_INDEX_BOOLEAN                 | 1       | NULL                  | 2673533 | Using where; Using index |
|  2 | DEPENDENT SUBQUERY | ref   | ref   | IX_GIANT_REF_INDEX_DATE,IX_GIANT_REF_COMPOUND_GIANT_ID_INTEGER | IX_GIANT_REF_COMPOUND_GIANT_ID_INTEGER | 8       | maihamadb.gi.GIANT_ID |       1 | Using where              |
+----+--------------------+-------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+---------+--------------------------+
2 rows in set (0.00 sec)

-- _/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
-- (MySQL-5.7.13) => 期待通り遅いけど、5.6より速くなってる
-- o でも実行計画はほとんど変わってない (DEPENDENT SUBQUERY 変わらず)
-- _/_/_/_/
+----------+
| count(*) |
+----------+
...
-- (MySQL-5.6.14) => 期待通り速い
-- o Start temporary, End temporary とな？
-- _/_/_/_/
+----------+
| count(*) |
+----------+
...
| id | select_type | table | type   | possible_keys                                                  | key                     | key_len | ref                    | rows | Extra                      |
+----+-------------+-------+--------+----------------------------------------------------------------+-------------------------+---------+------------------------+------+----------------------------+
|  1 | SIMPLE      | ref   | ref    | IX_GIANT_REF_INDEX_DATE,IX_GIANT_REF_COMPOUND_GIANT_ID_INTEGER | IX_GIANT_REF_INDEX_DATE | 3       | const                  |  152 | Start temporary            |
|  1 | SIMPLE      | gi    | eq_ref | PRIMARY                                                        | PRIMARY                 | 8       | maihamadb.ref.GIANT_ID |    1 | Using index; End temporary |
+----+-------------+-------+--------+----------------------------------------------------------------+-------------------------+---------+------------------------+------+----------------------------+
2 rows in set (0.00 sec)


-- _/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
-- (MySQL-5.7.13) => 期待通り速い (5.6と同じ)
-- o 速いの変わってないけど、5.6に比べて実行計画は変わっている。
-- _/_/_/_/
+----------+
| count(*) |
+----------+
...
--                                                                  ======================
--
-- 基点テーブル(外側)の絞り込みが強くて件数が少なく、サブクエリ(内側)の絞り込みが甘くて件数が多い場合
--
...
| id | select_type        | table | type  | possible_keys                                                  | key                                    | key_len | ref                   | rows | Extra                    |
+----+--------------------+-------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+------+--------------------------+
|  1 | PRIMARY            | gi    | range | IX_GIANT_INDEX_INTEGER                                         | IX_GIANT_INDEX_INTEGER                 | 4       | NULL                  | 1498 | Using where; Using index |
|  2 | DEPENDENT SUBQUERY | ref   | ref   | IX_GIANT_REF_INDEX_DATE,IX_GIANT_REF_COMPOUND_GIANT_ID_INTEGER | IX_GIANT_REF_COMPOUND_GIANT_ID_INTEGER | 8       | maihamadb.gi.GIANT_ID |    1 | Using where              |
+----+--------------------+-------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+------+--------------------------+
2 rows in set (0.00 sec)
...
-- o DEPENDENT SUBQUERY が向いてる検索とはいえ、semi join できるならそれに越したことはないって感じかな!?
-- _/_/_/_/
+----+-------------+-------+------------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+------+----------+-----------------------------+
| id | select_type | table | partitions | type  | possible_keys                                                  | key                                    | key_len | ref                   | rows | filtered | Extra                       |
+----+-------------+-------+------------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+------+----------+-----------------------------+
...
-- _/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
-- (MySQL-5.6.14) => おお、速い
-- o ref側で FirstMatch が登場している。(5.6から追加されたsemi joinの動作の一つ)
-- o SQLそのままの実行でなくSQLを書き換えてそう。(だが5.6では show warnings ができない)
-- o そういう意味では、5.5のときはどんなんだったか？知りたいところだが環境がない
-- _/_/_/_/
+----------+
| count(*) |
+----------+
...
| id | select_type | table | type  | possible_keys                                                  | key                                    | key_len | ref                   | rows | Extra                       |
+----+-------------+-------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+------+-----------------------------+
|  1 | SIMPLE      | gi    | range | PRIMARY,IX_GIANT_INDEX_INTEGER                                 | IX_GIANT_INDEX_INTEGER                 | 4       | NULL                  | 1498 | Using where; Using index    |
|  1 | SIMPLE      | ref   | ref   | IX_GIANT_REF_INDEX_DATE,IX_GIANT_REF_COMPOUND_GIANT_ID_INTEGER | IX_GIANT_REF_COMPOUND_GIANT_ID_INTEGER | 8       | maihamadb.gi.GIANT_ID |    1 | Using where; FirstMatch(gi) |
+----+-------------+-------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+------+-----------------------------+
2 rows in set (0.00 sec)


-- _/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
-- (MySQL-5.7.13) => おお、速い (5.6と同じ)
-- o 実行計画は5.6からまったく変わらず。
-- o show warnings で、サブクエリがsemi joinにされていて内側テーブルスキャンが回避されてるのがわかる。
-- o 5.6と変わらず FirstMatch があるので、semi joinの戦略として FirstMatch !?
-- o semi join自体は5.6から追加されたもののようなので、恐らく5.6も同じような挙動をしているはず。
-- _/_/_/_/
+----------+
| count(*) |
+----------+
...
-- _/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
-- (MySQL-8.0.36) => 変わらず速い
-- o 実行計画はまったく変わらず (FirstMatch, semi join)
-- _/_/_/_/
+----+-------------+-------+------------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+------+----------+-----------------------------+
| id | select_type | table | partitions | type  | possible_keys                                                  | key                                    | key_len | ref                   | rows | filtered | Extra                       |
+----+-------------+-------+------------+-------+----------------------------------------------------------------+----------------------------------------+---------+-----------------------+------+----------+-----------------------------+
